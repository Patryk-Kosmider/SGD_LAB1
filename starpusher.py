...
def main():
    global FPSCLOCK, DISPLAYSURF, IMAGESDICT, TILEMAPPING, OUTSIDEDECOMAPPING, BASICFONT, PLAYERIMAGES, currentImage, LEVELCOMPLETEFONT
    ...
    LEVELCOMPLETEFONT = pygame.font.Font('freesansbold.ttf', 42)
...
def runLevel(levels, levelNum):
    ...
    # Variables for level complete message
    ...
    while True: # main game loop
        ...
        if playerMoveTo != None and not levelIsComplete:
            ...
            if isLevelFinished(levelObj, gameStateObj):
                ...
                levelCompleteStartTime = pygame.time.get_ticks() # Record the time when level is completed
        ...
        if levelIsComplete:
            ...
            if levelCompleteDisplayed:
                completeSurf = LEVELCOMPLETEFONT.render('Level Complete!', 1, GREEN)
                completeRect = completeSurf.get_rect()
                completeRect.center = (HALF_WINWIDTH, HALF_WINHEIGHT + 100)
                DISPLAYSURF.blit(completeSurf, completeRect)

                if pygame.time.get_ticks() - levelCompleteStartTime >= 3000:
                    levelCompleteDisplayed = False
                    return 'solved'
